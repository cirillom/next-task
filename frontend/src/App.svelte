<script lang="ts">
  ...
  type View = 'next' | 'tasks' | 'tags' | 'workspaces' | 'settings';
  const views: View[] = ['next', 'tasks', 'tags', 'workspaces', 'settings'];
    ...
    <main class="content">
      ...
      {:else}
        {#key `${workspace.id}-${view}-${refreshKey}`}
          {#if view === 'next'}<Next {workspace} on:openTask={(event) => (editorTaskId = event.detail)} />
